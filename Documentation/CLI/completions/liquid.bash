...
_liquid() {
    ...
    __liquid_offer_flags_options 0
    ...
    *)
        ...
        COMPREPLY+=($(compgen -W 'render analyze batch validate benchmark init help' -- "${cur}"))
        ;;
    esac
}
...
_liquid_render() {
    repeating_flags=()
    non_repeating_flags=(--metrics --version -h --help)
    repeating_options=()
    non_repeating_options=(--context --output -o)
    __liquid_offer_flags_options 1

    # Offer option value completions
    case "${prev}" in
    '--context')
        __liquid_add_completions -o plusdirs -fX '!*.@(json)'
        return
        ;;
    '--output'|'-o')
        __liquid_add_completions -f
        return
        ;;
    esac
    ...
    1)
        __liquid_add_completions -o plusdirs -fX '!*.@(liquid|html)'
        return
        ;;
    esac
}
...
_liquid_analyze() {
    ...
    repeating_options=()
    non_repeating_options=(--format)
    __liquid_offer_flags_options 1

    # Offer option value completions
    case "${prev}" in
    '--format')
        __liquid_add_completions -W 'terminal'$'\n''json'$'\n''markdown'
        return
        ;;
    esac
...
_liquid_batch() {
    repeating_flags=()
    non_repeating_flags=(--continue-on-error --version -h --help)
    repeating_options=()
    non_repeating_options=(--context-dir --output -o --concurrency)
    __liquid_offer_flags_options 1

    # Offer option value completions
    case "${prev}" in
    '--context-dir')
        __liquid_add_completions -d
        return
        ;;
    '--output'|'-o')
        __liquid_add_completions -d
...
_liquid_validate() {
    repeating_flags=()
    non_repeating_flags=(--verbose --version -h --help)
    repeating_options=()
...
_liquid_benchmark() {
    ...
    non_repeating_flags=(--version -h --help)
    repeating_options=()
    non_repeating_options=(--iterations -i --context)
    __liquid_offer_flags_options 1
...
_liquid_init() {
    repeating_flags=()
    non_repeating_flags=(--git --version -h --help)
    repeating_options=()
    non_repeating_options=(--type)
    __liquid_offer_flags_options 1

    # Offer option value completions
    case "${prev}" in
    '--type')
        __liquid_add_completions -W 'simple'$'\n''web'$'\n''api'$'\n''documentation'
        return
        ;;
    esac
}
...
complete -o filenames -F _liquid liquid
